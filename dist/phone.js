...
function phLoadCfg() {
  try {
    return {
      ...PH_DEFAULT_CFG,
      ...JSON.parse(P.localStorage.getItem(PH_CFG) || "{}"),
    };
  } catch (e) {
    return { ...PH_DEFAULT_CFG };
  }
}
function phSaveCfg() {
  try {
...
function phViewSettings(stat) {
  ...
  const opt = (v, cur, label) =>
    ...
    "</option>";
  return (
    '<div class="scroll form">'+phThemeControl()+'<button class="card" data-act="go" data-view="appearance"><b>壁纸与聊天头像</b><p>四款预设壁纸 · 本地相册 · 专属头像</p></button><label class="sw"><input type="checkbox" data-cfg-bool="lowPower" '+(S.cfg.lowPower!==false?'checked':'')+'> 轻量模式（推荐）</label><p class="hint">关闭装饰动画与重阴影；相册压缩、按需加载和隐藏时不重绘始终启用。</p><div class="sect">手机 API</div>' +
...
function phMount() {
  ...
  sh.innerHTML =
    "<style>" +
    PH_CSS +
    '</style><div class="ball" role="button" aria-label="打开灵讯手机" title="灵讯手机">📱</div><div class="phone" hidden></div>';
  S.root = host;
  S.shadow = sh;
  ...
  // 拖动悬浮球
  const ball = sh.querySelector(".ball");
  let drag = null;
  ball.addEventListener("pointerdown", (e) => {
    const r = ball.getBoundingClientRect();
    drag = { x: e.clientX, y: e.clientY, l: r.left, t: r.top, moved: false };
  ...
  ball.addEventListener("pointermove", (e) => {
    ...
    drag.moved = true;
    phPlaceBall(drag.l + dx, drag.t + dy);
  });
  ball.tabIndex=0;ball.addEventListener("keydown",e=>{if(e.key==="Enter"||e.key===" "){e.preventDefault();phToggle();}});
  ...
  ball.addEventListener("pointerup", () => {
    ...
    if (moved) {
      const r = ball.getBoundingClientRect();
      S.cfg.ball = { x: r.left / P.innerWidth, y: r.top / P.innerHeight };
      phSaveCfg();
    } else phToggle();
  });
  const pos = S.cfg.ball;
  if (pos) phPlaceBall(pos.x * P.innerWidth, pos.y * P.innerHeight);
  else phPlaceBall(P.innerWidth - 70, P.innerHeight - 150);
...
function phOnResize() {
  ...
  if (b) {
    ...
    phPlaceBall(r.left, r.top);
  }
}
function phPlaceBall(x, y) {
  const b = S.shadow.querySelector(".ball");
  b.style.left = phClamp(x, 4, P.innerWidth - 56) + "px";
  b.style.top = phClamp(y, 4, P.innerHeight - 56) + "px";
}
function phUpdateBall() {
  const b = S.shadow?.querySelector(".ball");
  if (!b) return;
  b.style.display = S.cfg.showBall && !S.open ? "flex" : "none";
  const n = phUnread();
...
function phToggle(force){
  ...
  phRememberView();S.open=next;S.shadow.querySelector('.phone').hidden=!next;phLiveSync();
...
function phRenderNow(){
 ...
 const html='<div class="sbar"><span>'+phEsc(phClock(stat))+'</span><span class="camera" aria-hidden="true"></span><div class="system-icons"><span>5G</span><svg viewBox="0 0 24 12" aria-hidden="true"><path d="M1 11V8h2v3zm4 0V6h2v5zm4 0V3h2v8zm4 0V1h2v10" fill="currentColor"/><rect x="18" y="2" width="4" height="8" rx="1" fill="currentColor"/></svg><button data-act="close" title="收起手机" aria-label="收起手机">'+phIcon('close')+'</button></div></div>'+(home?'':'<div class="nav"><button class="nav-back" data-act="back" aria-label="返回">'+phIcon('back')+'</button><b>'+phEsc(title)+'</b><button data-act="home" aria-label="手机主屏">'+phIcon('home')+'</button></div>')+'<div class="body'+(scroller?'':' scroll')+'" data-page="'+v+'">'+body+'</div>'+(S.err?'<div class="err" role="alert"><span>'+phEsc(S.err)+'</span><button data-act="clearErr" aria-label="关闭提示">×</button></div>':'')+(home?phDock():'')+'<button class="home-indicator" data-act="home" aria-label="返回主屏"><span></span></button>'+(S.modal?'<div class="modal" role="dialog" aria-modal="true"><div class="box">'+S.modal.html+'</div></div>':'');
...
const phPublic={version:3,open:()=>phToggle(),toggle:()=>phToggle(),isOpen:()=>S.open};
